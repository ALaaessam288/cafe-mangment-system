      ...
      {loading ? (
        ...
        <div className="daily-report-modal" style={{ maxHeight: '82vh', overflowY: 'auto', paddingInlineEnd: '4px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '10px', marginBottom: '16px' }}>
            <div style={{ background: 'linear-gradient(135deg, #20212d, #11131a)', color: '#fff', padding: '12px', borderRadius: '10px', border: '1px solid #45465a' }}>
              <div style={{ fontSize: '12px', color: '#9995a8', display: 'flex', alignItems: 'center', gap: '4px' }}><TrendingUp size={14} /> إجمالي مبيعات الشيفت</div>
              <div style={{ fontSize: '20px', fontWeight: '900', marginTop: '4px', color: '#c9c1ff' }}>{formatCurrency(totalRevenue)}</div>
            </div>
            ...
            <div style={{ background: 'var(--bg-surface)', padding: '12px', borderRadius: '10px', border: '1px solid var(--border-color)' }}>
              ...
              <div style={{ fontSize: '17px', fontWeight: '800', marginTop: '4px', color: '#a99cff' }}>{formatCurrency(foodRevenue)}</div>
            </div>
            ...
            <div style={{ background: 'var(--bg-surface)', padding: '12px', borderRadius: '10px', border: '1px solid var(--border-color)' }}>
              ...
              <div style={{ fontSize: '17px', fontWeight: '800', marginTop: '4px', color: '#8e82eb' }}>{formatCurrency(buffetRevenue)}</div>
            </div>

            <div style={{ background: 'var(--bg-surface)', padding: '12px', borderRadius: '10px', border: '1px solid var(--border-color)' }}>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>🍿 صافي السناكس والحلويات</div>
              <div style={{ fontSize: '17px', fontWeight: '800', marginTop: '4px', color: '#c9c1ff' }}>{formatCurrency(snacksNet)}</div>
            </div>

            <div style={{ background: 'var(--bg-surface)', padding: '12px', borderRadius: '10px', border: '1px solid var(--border-color)' }}>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>💸 إجمالي المصاريف</div>
              <div style={{ fontSize: '17px', fontWeight: '800', marginTop: '4px', color: '#e56273' }}>-{formatCurrency(totalExpenses)}</div>
            </div>

            <div style={{ background: 'var(--bg-surface)', padding: '12px', borderRadius: '10px', border: '1px solid var(--border-color)' }}>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>👥 سُلف الموظفين المسحوبة</div>
              <div style={{ fontSize: '17px', fontWeight: '800', marginTop: '4px', color: '#c084fc' }}>-{formatCurrency(totalEmployeeAdvances)}</div>
            </div>
          </div>

          {/* Section 2: Order Status & Types Stats */}
          <div style={{ background: 'rgba(32, 33, 45, 0.4)', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '14px', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '8px', fontSize: '12px' }}>
              ...
              <div style={{ background: 'var(--bg-secondary)', padding: '8px', borderRadius: '8px', textAlign: 'center' }}>
                <span style={{ color: '#64d7bd' }}>المدفوعة (محصلة)</span>
                <div style={{ fontSize: '16px', fontWeight: '900', color: '#64d7bd' }}>{completedOrdersCount}</div>
              </div>
              <div style={{ background: 'var(--bg-secondary)', padding: '8px', borderRadius: '8px', textAlign: 'center' }}>
                <span style={{ color: '#a99cff' }}>المفتوحة الآن</span>
                <div style={{ fontSize: '16px', fontWeight: '900', color: '#a99cff' }}>{openOrdersCount}</div>
              </div>
              <div style={{ background: 'var(--bg-secondary)', padding: '8px', borderRadius: '8px', textAlign: 'center' }}>
                <span style={{ color: '#e56273' }}>الملغاة (Voided)</span>
                <div style={{ fontSize: '16px', fontWeight: '900', color: '#e56273' }}>{voidedOrdersCount}</div>
              </div>
              <div style={{ background: 'var(--bg-secondary)', padding: '8px', borderRadius: '8px', textAlign: 'center' }}>
          ...
          <div style={{ background: 'var(--bg-secondary)', border: '1.5px solid var(--border-color)', borderRadius: '10px', padding: '14px', marginBottom: '16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '8px', marginBottom: '12px' }}>
              <div style={{ padding: '8px 10px', background: 'rgba(22, 163, 74, 0.1)', border: '1px solid rgba(22, 163, 74, 0.25)', borderRadius: '8px' }}>
                ...
                <div style={{ fontSize: '15px', fontWeight: '900', color: '#64d7bd' }}>{formatCurrency(totalCash)}</div>
              </div>
              <div style={{ padding: '8px 10px', background: 'rgba(14, 165, 233, 0.1)', border: '1px solid rgba(14, 165, 233, 0.25)', borderRadius: '8px' }}>
                ...
                <div style={{ fontSize: '15px', fontWeight: '900', color: '#8e82eb' }}>{formatCurrency(totalInstapay)}</div>
              </div>
              <div style={{ padding: '8px 10px', background: 'rgba(169, 156, 255, 0.1)', border: '1px solid rgba(169, 156, 255, 0.25)', borderRadius: '8px' }}>
                <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>📱 محافظ (Vodafone Cash)</span>
                <div style={{ fontSize: '15px', fontWeight: '900', color: '#a99cff' }}>{formatCurrency(totalWallet)}</div>
              </div>
              <div style={{ padding: '8px 10px', background: 'rgba(168, 85, 247, 0.1)', border: '1px solid rgba(168, 85, 247, 0.25)', borderRadius: '8px' }}>
                ...
                <div style={{ fontSize: '15px', fontWeight: '900', color: '#8e82eb' }}>{formatCurrency(totalCard)}</div>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '8px', fontSize: '12.5px', paddingTop: '8px', borderTop: '1px dashed var(--border-color)' }}>
              ...
              <div>تحصيل ديون قديمة: <strong style={{ color: '#64d7bd' }}>+{formatCurrency(totalCollectedDebts)}</strong></div>
              <div>المصاريف المسحوبة: <strong style={{ color: '#e56273' }}>-{formatCurrency(totalExpenses)}</strong></div>
              <div>سُلف الموظفين: <strong style={{ color: '#e56273' }}>-{formatCurrency(totalEmployeeAdvances)}</strong></div>
            </div>
            ...
            <div style={{ marginTop: '12px', paddingTop: '10px', borderTop: '2px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
              ...
              {shift.closedAt && (
                <div>
                  ...
                  <span style={{ marginInlineStart: '10px', fontSize: '14px', fontWeight: '900', color: shiftVariance >= 0 ? '#64d7bd' : '#e56273' }}>
                    ...
                    {inventoryAuditRecords.map((r) => {
                      ...
                      return (
                        <tr key={r.auditItemId}>
                          ...
                          <td style={{ fontWeight: '800', color: hasWaste ? '#e56273' : '#64d7bd' }}>
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {expenses.map((e) => (
                      <tr key={e.id}>
                        <td>{e.description}</td>
                        <td style={{ fontWeight: '800', color: '#e56273' }}>{formatCurrency(e.amount)}</td>
                        <td>
                          <span style={{ fontSize: '10px', padding: '2px 6px', borderRadius: '4px', background: e.status === 'PENDING_SETTLEMENT' ? 'rgba(169, 156, 255,0.2)' : 'rgba(22,163,74,0.2)', color: e.status === 'PENDING_SETTLEMENT' ? '#a99cff' : '#64d7bd' }}>
                  ...
                  <tbody>
                    {employeeMovements.map((m) => (
                      <tr key={m.id}>
                        ...
                        <td>
                          <span style={{ fontSize: '11px', padding: '1px 6px', borderRadius: '4px', background: m.type === 'ADVANCE' ? '#fce7f3' : m.type === 'DEDUCTION' ? '#fee2e2' : '#dcfce7', color: m.type === 'ADVANCE' ? '#be185d' : m.type === 'DEDUCTION' ? '#c45661' : '#15803d' }}>
