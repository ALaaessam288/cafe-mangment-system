...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    ...
    @Query("SELECT COALESCE(SUM(p.amount), 0) FROM Payment p WHERE p.order.shift.id = :shiftId AND p.method = 'CASH'")
    BigDecimal sumCashAmountByShiftId(@Param("shiftId") Long shiftId);
}